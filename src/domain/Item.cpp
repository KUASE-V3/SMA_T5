...
std::string Item::toString() const {
    ...
    oss << std::setw(2) << code << ") " << std::left << std::setw(20) << name
        << "     가격 : " << price << "    재고 : " << count;
    return oss.str();
}

Item::Item(const Item &other)
    : code(other.code), name(other.name), price(other.price), count(other.count) {}

Item::Item(Item &&other) noexcept
    : code(other.code), name(std::move(other.name)), price(other.price), count(other.count) {}
